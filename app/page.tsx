  ...
  Not sure where to go? <a className="underline" href="/wizard">Try the matcher →</a>
      ...
      <form className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3" action="/compare" aria-label="City comparison form">
        <div>
          ...
          <input name="a" list="city-slugs" defaultValue="washington-dc" required
                 className="mt-1 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-slate-900 placeholder-slate-400 shadow-sm focus:border-blue-500 focus:ring-blue-500"/>
        </div>
        <div>
        ...
        <div className="sm:col-span-3">
          <button className="w-full rounded-md bg-slate-900 px-4 py-2 text-white shadow-sm hover:bg-slate-800 focus:ring-2 focus:ring-blue-500">
            Compare
          </button>
        </div>
      </form>
      ...
      <div className="mt-10">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          {PRESETS.map(p => (
            <Link key={p.label}
              className="rounded-lg border border-slate-200 bg-white px-4 py-3 text-slate-800 shadow-sm hover:shadow transition"
