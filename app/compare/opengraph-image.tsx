...
export default function OG({ searchParams }: { searchParams: SP }) {
  ...
  const salary = searchParams.salary ? Number(searchParams.salary).toLocaleString() : "";

  const title = a && b ? `${capitalize(a)}  vs  ${capitalize(b)}` : "City comparison";
  const subtitle = salary ? `How does $${salary} feel if you move?` : SITE.tagline;
