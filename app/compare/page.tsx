...
export default function ComparePage({
  ...
  // ---------- Invalid city handling with friendly suggestions ----------
  if (!a || !b) {
      ...
      return (
        <ul className="mt-2 flex flex-wrap gap-2 text-sm">
          {suggestions.map((s) => {
            const href =
              forParam === "a"
                ? `/compare?a=${encodeURIComponent(s)}&b=${encodeURIComponent(
                    keepOther
                  )}&salary=${salaryStr}`
                : `/compare?a=${encodeURIComponent(
                    keepOther
                  )}&b=${encodeURIComponent(s)}&salary=${salaryStr}`;
            return (
              <li key={`${forParam}-${s}`}>
  ...
  // ---------- Spending power (salary * (RPP_A / RPP_B)) ----------
  const spend = spendingPower(salary, a.rpp, b.rpp);
  const direction = spend.deltaPct >= 0 ? "increases" : "decreases";
  const deltaStr = `${spend.deltaPct >= 0 ? "+" : ""}${spend.deltaPct.toFixed(
    1
  )}%`;

  const rows = [
    {
      label: "Affordability (RPP, lower is cheaper)",
      a: a.rpp,
      b: b.rpp,
      better: "lower" as const,
    },
    {
      label: "Rent Index (higher is pricier)",
      a: a.rentIndex,
      b: b.rentIndex,
      better: "lower" as const,
    },
    ...
    {
      label: "Diversity Index (0–1)",
      a: a.diversityIndex,
      b: b.diversityIndex,
      better: "higher" as const,
    },
    {
      label: "Internet Median Mbps",
      a: a.internetMbps,
      b: b.internetMbps,
      better: "higher" as const,
    },
  ...
  const briefAB = `/brief?a=${a.slug}&b=${b.slug}&salary=${salary}`;
  const briefBA = `/brief?a=${b.slug}&b=${a.slug}&salary=${salary}`;
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-12 text-slate-900">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-3xl font-semibold text-slate-900">
            {a.name}, {a.state} <span className="text-slate-500">vs</span>{" "}
            {b.name}, {b.state}
          </h1>
          <p className="mt-2 text-sm text-slate-600">
            Salary entered:{" "}
            <strong className="text-slate-900">{fmtMoney(salary)}</strong>. Moving
            from {a.name} to {b.name}, your spending power{" "}
            <strong
              className={
                spend.deltaPct >= 0 ? "text-green-700" : "text-red-700"
              }
            >
              {direction} by {deltaStr}
            </strong>{" "}
            — that’s like{" "}
            <strong className="text-slate-900">
              {fmtMoney(spend.destEquivalent)}
            </strong>{" "}
            in {b.name}.
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <a href={briefAB} className="btn-outline">Brief: move to {b.name}</a>
          <a href={briefBA} className="btn-outline">Brief: move to {a.name}</a>
          <CopyLinkButton />
        </div>
      </div>
      ...
      <div className="card">
          ...
          <div className="card-muted p-4">
            ...
            <div className="mt-1 text-sm text-slate-600">
              ...
              <DeltaPill
                ...
                good={spend.destEquivalent >= salary}
              />
            </div>
          </div>
        ...
        <div className="overflow-x-auto border-t border-slate-200">
          <table className="table">
            ...
            <tbody>
              {rows.map((r) => {
                const delta = percentDelta(r.a as number, r.b as number); // B vs A
                const good =
                  (r.better === "lower" && delta <= 0) ||
                  (r.better === "higher" && delta >= 0);
                const format = (v: number) =>
                  r.money ? fmtMoney(v) : v.toLocaleString();
                return (
                  <tr key={r.label}>
      ...
      {/* NEW: Briefs section beneath the table, full width of the main container */}
      <section className="mt-8 card p-6">
        <h2 className="text-lg font-semibold text-slate-900">View relocation briefs</h2>
        <p className="mt-1 text-sm text-slate-600">
          One-page summaries you can print or save as PDF.
        </p>
        <div className="mt-4 flex flex-col gap-2 sm:flex-row">
          {/* Keep the user’s original A → B order first */}
          <a href={briefAB} className="btn w-full sm:w-auto">
            Brief: {a.name} → {b.name}
          </a>
          <a href={briefBA} className="btn-outline w-full sm:w-auto">
            Brief: {b.name} → {a.name}
          </a>
        </div>
      </section>
