...
export default function BriefsPage() {
  ...
  const results = React.useMemo(() => {
    ...
    if (!s) return [];
    // Simple, fast filter on name/state/slug
    return CITIES
      .filter((c) => {
        const hay = `${c.name}, ${c.state} ${c.slug}`.toLowerCase();
        return hay.includes(s);
      })
      .slice(0, 25); // keep it snappy
  }, [q]);
    ...
    <main className="mx-auto max-w-5xl px-6 py-12 text-slate-900">
      ...
      <section className="mt-6 card p-6">
        ...
        <div>
          ...
          />
          <div className="mt-1 text-xs text-slate-500">Used to compute cost-of-living adjusted spending power.</div>
        </div>
